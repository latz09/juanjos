...
const Review = ({ review, name }) => {
	let x = [0, 1, 2, 3, 4];
	return (
		<div className='grid gap-2 mx-auto p-8 h-full hover:scale-105  transition duration-700 '>
			<h2 className='text-lg lg:text-3xl'>{name}</h2>
			<div className='flex space-x-2'>
				{x.map((star, index) => (
					<span key={star} className='lg:text-xl'>
						{' '}
						<motion.div
							initial={{ opacity: 0.7 }}
							animate={{ opacity: 1 }}
